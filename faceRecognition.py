...
def faceDetection(test_img):
    ...
    face_haar_cascade=cv2.CascadeClassifier('H:\\ML faceDetection\\haarcascade_frontalface_default.xml')
...
def lebels_for_training_data(directory):
    ...
    for path,subdirnames,filenames in os.walk(directory):
        for filename in filenames:
            ...
            if len(faces_rect)!=1:
                continue # since we areassuming only single person images are using for classifier
            (x,y,w,h)=faces_rect[0]
            roi_gray=gray_img[y:y+w,x:x+h]
            faces.append(roi_gray)
            faceID.append(int(id))
    return faces,np.array(faceID)
